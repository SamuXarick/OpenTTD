 ...
 * height or at some offset (TREE_PLANT_HEIGHT_OFFSET units) of it.
 *
...
static void PlaceTreesAtSameHeight(TileIndex tile)
{
	...
	/* Above the snowline, plant more trees. */
	if (_settings_game.game_creation.landscape == LandscapeType::Arctic && ht > GetSnowLine()) {
		j *= 3;
	}

	/* Scale generation by maximum map height. */
	uint8_t max_height = _settings_game.construction.map_height_limit;
	if (max_height > MAP_HEIGHT_LIMIT_ORIGINAL) {
		j *= MAP_HEIGHT_LIMIT_ORIGINAL / max_height;
	}

	/* Define the range around the existing tree. */
	int x = TileX(tile);
	int y = TileY(tile);
	...
	if (std::max({ abs(hs - he), abs(hs - ht), abs(he - ht) }) <= TREE_PLANT_HEIGHT_OFFSET || j < 66) {
		...
		if (j > 100) {
			j = 100 + 100 * (j - 100) / (2 * 3 * MAX_TILE_HEIGHT - 100);
		}
