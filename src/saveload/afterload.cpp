...
bool AfterLoadGame()
{
			...
			case MP_STATION: {
				...
				switch (GetStationType(t)) {
					...
					case STATION_OILRIG: {
						 ...
						 * the map
						 */
						TileIndex t1 = t + TileIndexDiffC(0, 1);
	...
	if (IsSavegameVersionBefore(SLV_164) && !IsSavegameVersionBefore(SLV_32)) {
		...
		for (auto t : Map::Iterate()) {
			...
			if (fence != 0 && IsTileType(t + TileIndexDiffC(1, 0), MP_CLEAR) && IsClearGround(t + TileIndexDiffC(1, 0), CLEAR_FIELDS)) {
				SetFence(t + TileIndexDiffC(1, 0), DIAGDIR_NE, fence);
			}
			fence = GB(t.m4(), 2, 3);
			if (fence != 0 && IsTileType(t + TileIndexDiffC(0, 1), MP_CLEAR) && IsClearGround(t + TileIndexDiffC(0, 1), CLEAR_FIELDS)) {
				SetFence(t + TileIndexDiffC(0, 1), DIAGDIR_NW, fence);
			}
			SB(t.m4(), 2, 3, 0);
			SB(t.m4(), 5, 3, 0);
		}
	...
	if (IsSavegameVersionBefore(SLV_SERVE_NEUTRAL_INDUSTRIES)) {
		...
		for (Station *st : Station::Iterate()) {
			if (IsTileType(st->xy, MP_STATION) && IsOilRig(st->xy)) {
				/* Industry tile is always adjacent during construction by TileIndexDiffC(0, 1) */
				st->industry = Industry::GetByTile(st->xy + TileIndexDiffC(0, 1));
