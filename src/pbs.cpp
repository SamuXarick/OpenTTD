...
void SetRailStationPlatformReservation(TileIndex start, DiagDirection dir, bool b)
{
	...
	TileOffset diff = TileOffsByDiagDir(dir);
	...
	while (ft.Follow(tile, trackdir)) {
		...
		if (reserved == TRACKDIR_BIT_NONE) {
			if (ft.is_station) {
				...
				TileOffset diff = TileOffsByDiagDir(ft.exitdir);
...
PBSTileInfo FollowTrainReservation(const Train *v, Vehicle **train_on_res)
{
	...
	if (train_on_res != nullptr) {
		...
		if (*train_on_res == nullptr && IsRailStationTile(ftoti.res.tile)) {
			...
			TileOffset diff = TileOffsByDiagDir(TrackdirToExitdir(ReverseTrackdir(ftoti.res.trackdir)));
