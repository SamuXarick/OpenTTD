...
static void UpdateFences(TileIndex tile)
{
	...
	for (DiagDirection dir = DIAGDIR_BEGIN; dir < DIAGDIR_END; dir++) {
		...
		TileIndex neighbour = AddTileIndexDiffC(tile, TileIndexDiffCByDiagDir(dir));
...
{
	for (DiagDirection dir = DIAGDIR_BEGIN; dir < DIAGDIR_END; dir++) {
		TileIndex t = AddTileIndexDiffC(tile, TileIndexDiffCByDiagDir(dir));
	...
	do {
		...
		if (IsTileType(tile, MP_CLEAR) && !IsClearGround(tile, CLEAR_DESERT)) {
			...
			for (;;) {
				...
				do {
					if (--j == 0) goto get_out;
					tile_new = AddTileIndexDiffC(tile, TileIndexDiffCByDiagDir((DiagDirection)GB(Random(), 0, 2)));
