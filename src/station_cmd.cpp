...
CommandCost CmdBuildRailStation(DoCommandFlag flags, TileIndex tile_org, RailType rt, Axis axis, uint8_t numtracks, uint8_t plat_len, StationClassID spec_class, uint16_t spec_index, StationID station_to_join, bool adjacent)
{
	...
	if (flags & DC_EXEC) {
		...
		TileIndexDiffC tile_delta = TileIndexDiffCByDiagDir(AxisToDiagDir(axis));
		TileIndexDiffC track_delta = TileIndexDiffCByDiagDir(AxisToDiagDir(OtherAxis(axis)));
		...
		do {
			...
			tile_track = AddTileIndexDiffC(tile_track, track_delta);
		} while (--numtracks);
