...
restart:
	...
	if (ta.w != 0 && ta.h != 0) {
		...
		for (uint i = 0; !func(st, ta.tile + TileIndexDiffC(0, i));) {
			/* the left side is unused? */
			if (++i == ta.h) {
				ta.tile += TileIndexDiffC(1, 0);
				ta.w--;
				goto restart;
			}
		}
		...
		for (uint i = 0; !func(st, ta.tile + TileIndexDiffC(ta.w - 1, i));) {
		...
		for (uint i = 0; !func(st, ta.tile + TileIndexDiffC(i, 0));) {
			/* the left side is unused? */
			if (++i == ta.w) {
				ta.tile += TileIndexDiffC(0, 1);
				ta.h--;
				goto restart;
			}
		}
		...
		for (uint i = 0; !func(st, ta.tile + TileIndexDiffC(i, ta.h - 1));) {
			/* the left side is unused? */
			if (++i == ta.w) {
				ta.h--;
				goto restart;
...
CommandCost CmdBuildDock(DoCommandFlag flags, TileIndex tile, StationID station_to_join, bool adjacent)
{
	...
	TileIndex tile_cur = tile + direction;
	...
	if (flags & DC_EXEC) {
		...
		TileIndex flat_tile = tile + direction;
...
static TileIndex FindDockLandPart(TileIndex t)
{
	...
	for (DiagDirection d = DIAGDIR_BEGIN; d != DIAGDIR_END; d++) {
		...
		if (GetStationGfx(tile) < GFX_DOCK_BASE_WATER_PART && tile + GetDockDirection(tile) == t) return tile;
	}
...
static CommandCost RemoveDock(TileIndex tile, DoCommandFlag flags)
{
	...
	TileIndex tile2 = tile1 + GetDockDirection(tile1);
...
draw_default_foundation:
	...
	if (IsBuoy(ti->tile)) {
		...
		if (ti->tileh == SLOPE_FLAT) {
			...
			TileIndex water_tile = ti->tile + GetDockDirection(ti->tile);
