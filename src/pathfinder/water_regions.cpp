...
public:
	...
	void ForceUpdate()
	{
		...
		if (this->NumberOfPatches() == 0 || (this->NumberOfPatches() == 1 &&
				std::all_of(this->data.tile_patch_labels.get(), this->data.tile_patch_labels.get() + CurrentWaterRegionNumberOfTiles(), [](TWaterRegionPatchLabel label) { return label == FIRST_REGION_LABEL; }))) {
	...
	void PrintDebugInfo()
	{
		...
		std::string traversability = fmt::format("{:0{}b}", this->GetEdgeTraversabilityBits(DIAGDIR_NW), CurrentWaterRegionEdgeLength());
		...
		Debug(map, 9, "  +{:->{}}+", "", CurrentWaterRegionEdgeLength() * (max_element_width + 1) + 1);
		traversability = fmt::format("{:0{}b}", this->GetEdgeTraversabilityBits(DIAGDIR_SE), CurrentWaterRegionEdgeLength());
		Debug(map, 9, "    {:{}}", fmt::join(traversability, " "), max_element_width);
	}
};
