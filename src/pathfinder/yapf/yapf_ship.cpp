...
public:
		...
		bool &path_found, ShipPathCache &path_cache, Trackdir &best_origin_dir)
	{
		...
		std::vector<WaterRegionPatchDesc> high_level_path = YapfShipFindWaterRegionPath(v, tile, number_of_water_regions_lookahead + 1, dest_tiles);
		...
		for (int attempt = 0; attempt < 2; ++attempt) {
			...
			const std::span<WaterRegionPatchDesc> high_level_path_span(high_level_path.data(), std::min<size_t>(high_level_path.size(), number_of_water_regions_lookahead + 1));
			const bool is_intermediate_destination = static_cast<int>(high_level_path_span.size()) >= number_of_water_regions_lookahead + 1;
			...
			while (node->parent) {
				...
				const bool node_water_patch_on_high_level_path = std::find(high_level_path_span.begin(), high_level_path_span.end(), node_water_patch) != high_level_path_span.end();
...
{
	explicit CYapfShip1(int max_nodes) { this->max_search_nodes = max_nodes; }
};

/* YAPF type 2 - uses TileIndex/DiagDirection as Node key */
struct CYapfShip2 : CYapfT<CYapfShip_TypesT<CYapfShip2, CFollowTrackWater, CShipNodeListExitDir > >
{
	explicit CYapfShip2(int max_nodes) { this->max_search_nodes = max_nodes; }
};
