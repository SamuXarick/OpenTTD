...
Track YapfShipChooseTrack(const Ship *v, TileIndex tile, bool &path_found, ShipPathCache &path_cache)
{
	...
	typedef Trackdir (*PfnChooseShipTrack)(const Ship *, TileIndex, TrackdirBits, TrackdirBits, bool &path_found, ShipPathCache &path_cache, Trackdir &best_origin_dir);
	...
	const Trackdir td_ret = pfnChooseShipTrack(v, tile, origin_dirs, TRACKDIR_BIT_NONE, path_found, path_cache, best_origin_dir);
...
{
	/* default is YAPF type 2 */
	typedef bool (*PfnCheckShipReverse)(const Ship *, Trackdir *);
	/* check if non-default YAPF type needed */
	...
	return pfnCheckShipReverse(v, trackdir);
}
