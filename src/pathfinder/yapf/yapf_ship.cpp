...
public:
	...
	{
		m_water_region_corridor.clear();
		for (const HighNode &path_entry : path) m_water_region_corridor.push_back(std::get<1>(path_entry));
	}
	 ...
	 * @param tile overriding tile if not using current's ship position.
	 ...
	 */
	static inline FindDepotData FindNearestDepot(const Ship *v, int max_penalty, TileIndex tile, TrackdirBits trackdirs)
	{
		FindDepotData depot;

		std::vector<HighNode> high_level_path = YapfFindShipDepotRegionPath(v, tile, max_penalty);

		/* 'tile' and 'trackdirs' contain information about where the ship would be located
		 * ahead of its current position in the near future when the call is not top-level.
		 * If such information is invalid, then that makes this the top call, so the ship's
		 * current position and trackdirs need to be restored. */
		...
		if (is_top_level_call) {
			tile = v->tile;
			trackdirs = TrackdirToTrackdirBits(td) | TrackdirToTrackdirBits(td_rev);
		}
		assert(IsValidTrackdir(FindFirstTrackdir(trackdirs)));

		/* Handle special case - when tile is a ship depot tile. */
		if (IsShipDepotTile(tile) && GetShipDepotPart(tile) == DEPOT_PART_NORTH && IsTileOwner(tile, v->owner)) {
			depot.tile = tile;
			depot.best_length = 0;
		...
		for (int attempt = 0; attempt < 2; attempt++) {
			...
			if (attempt == 0 && !path_found && !high_level_path.empty()) continue; // Try again with restricted search area.

			if (!path_found) { // No depot found by low level pathfinder.
				/**
				 * This assert can't fail. A depot is already spotted at the top level.
				 * The secondary level should always find a path, garanteed!
				 */
				assert(is_top_level_call);

				if (high_level_path.empty()) break; // Returns no depot found at all.

				/* Set the cost of the best path and its depot. */
				HighNode hNode = high_level_path.back();
				depot.tile = pf.GetShipDepotDestination(high_level_path.back());
				depot.best_length = std::get<2>(hNode);
			...
			/* When the initial position is the ship, we can determine whether to reverse. */
			if (is_top_level_call) {
...
public:
	inline TileIndex GetShipDepotDestination(const HighNode &hnode)
	{
		...
		return GetShipDepotInWaterRegionPatch(GetShipDepot, std::get<0>(hnode), std::get<1>(hnode), Yapf().GetVehicle());
	}
...
FindDepotData YapfShipFindNearestDepot(const Ship *v, int max_penalty, TileIndex tile, TrackdirBits dirs)
{
	return CYapfShipAnyDepot::FindNearestDepot(v, max_penalty, tile, dirs);
}
