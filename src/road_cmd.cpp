...
CommandCost CheckAllowRemoveRoad(TileIndex tile, RoadBits remove, Owner owner, RoadTramType rtt, DoCommandFlag flags, bool town_check)
{
	...
	if ((present & ROAD_NE) && (GetAnyRoadBits(tile + TileIndexDiffC(-1,  0), rtt) & ROAD_SW)) n |= ROAD_NE;
	if ((present & ROAD_SE) && (GetAnyRoadBits(tile + TileIndexDiffC( 0,  1), rtt) & ROAD_NW)) n |= ROAD_SE;
	if ((present & ROAD_SW) && (GetAnyRoadBits(tile + TileIndexDiffC( 1,  0), rtt) & ROAD_NE)) n |= ROAD_SW;
	if ((present & ROAD_NW) && (GetAnyRoadBits(tile + TileIndexDiffC( 0, -1), rtt) & ROAD_SE)) n |= ROAD_NW;
