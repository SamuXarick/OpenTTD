...
struct ScriptListWindow : public Window {
	...
	void ChangeScript()
	{
		if (_game_mode == GM_NORMAL && ((this->slot == OWNER_DEITY && Game::GetInstance() != nullptr) || Company::IsValidAiID(this->slot))) return;
		if (this->selected == -1) {
		...
		InvalidateWindowData(WC_SCRIPT_SETTINGS, this->slot);
...
struct ScriptSettingsWindow : public Window {
	...
	void RebuildVisibleSettings()
	{
		...
		this->visible_settings = GetVisibleSettingsList(this->slot);
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		switch (widget) {
			...
			case WID_SCRS_RESET:
				this->script_config->ResetEditableSettings(_game_mode != GM_NORMAL || IsConsideredDead(this->slot));
				this->SetDirty();
				break;
		}
	}
