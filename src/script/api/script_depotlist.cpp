...
ScriptDepotList::ScriptDepotList(ScriptTile::TransportType transport_type)
{
			...
			for (const Station *st : Station::Iterate()) {
				if (is_deity || st->owner == owner) {
					for (uint i = 0; i < st->airport.GetNumHangars(); i++) {
						this->AddItemUnchecked(st->airport.GetHangarTile(i).base());
					}
				}
			}
			return;
	...
	for (const Depot *depot : Depot::Iterate()) {
		if ((is_deity || ::GetTileOwner(depot->xy) == owner) && ::IsTileType(depot->xy, tile_type)) this->AddItemUnchecked(depot->xy.base());
	}
}
