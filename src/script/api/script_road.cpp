...
/* static */ bool ScriptRoad::IsDriveThroughRoadStationTile(TileIndex tile)
{
	...
	return ::IsDriveThroughStopTile(tile) && ::IsStationRoadStopTile(tile) && HasBit(::GetPresentRoadTypes(tile), (::RoadType)GetCurrentRoadType());
}
...
/* static */ bool ScriptRoad::IsRoadWaypointTile(TileIndex tile)
{
	...
	return ::IsRoadWaypointTile(tile) && HasBit(::GetPresentRoadTypes(tile), static_cast<::RoadType>(GetCurrentRoadType()));
}
...
/* static */ bool ScriptRoad::BuildRoadWaypoint(TileIndex tile, StationID waypoint_id)
{
	...
	StationID to_join = ScriptWaypoint::IsValidWaypoint(waypoint_id) ? waypoint_id : ::INVALID_STATION;
...
/* static */ Money ScriptRoad::GetBuildCost(RoadType roadtype, BuildType build_type)
{
	...
	switch (build_type) {
		...
		case BT_TRUCK_STOP:
		case BT_WAYPOINT: return ::GetPrice(PR_BUILD_STATION_TRUCK, 1, nullptr);
		default: return -1;
	}
}
