...
static void DrawWaterEdges(bool canal, uint offset, TileIndex tile)
{
	...
	wa  = IsWateredTile(tile + TileIndexDiffC(-1,  0), DIR_SW) << 0;
	wa += IsWateredTile(tile + TileIndexDiffC( 0,  1), DIR_NW) << 1;
	wa += IsWateredTile(tile + TileIndexDiffC( 1,  0), DIR_NE) << 2;
	wa += IsWateredTile(tile + TileIndexDiffC( 0, -1), DIR_SE) << 3;
	...
	switch (wa & 0x03) {
		...
		case 3: if (!IsWateredTile(tile + TileIndexDiffC(-1, 1), DIR_W)) DrawWaterSprite(base, offset + 8, feature, tile); break;
	}

	/* bottom corner */
	switch (wa & 0x06) {
		case 0: DrawWaterSprite(base, offset + 5, feature, tile); break;
		case 6: if (!IsWateredTile(tile + TileIndexDiffC(1, 1), DIR_N)) DrawWaterSprite(base, offset + 9, feature, tile); break;
	}

	/* left corner */
	switch (wa & 0x0C) {
		case  0: DrawWaterSprite(base, offset + 6, feature, tile); break;
		case 12: if (!IsWateredTile(tile + TileIndexDiffC(1, -1), DIR_E)) DrawWaterSprite(base, offset + 10, feature, tile); break;
	}

	/* upper corner */
	switch (wa & 0x09) {
		case 0: DrawWaterSprite(base, offset + 7, feature, tile); break;
		case 9: if (!IsWateredTile(tile + TileIndexDiffC(-1, -1), DIR_S)) DrawWaterSprite(base, offset + 11, feature, tile); break;
	}
}
