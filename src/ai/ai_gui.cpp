...
struct AIConfigWindow : public Window {
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		for (CompanyID i = COMPANY_FIRST; i < MAX_COMPANIES; i++) {
			if (IsEligible(i)) {
			...
			} else if (Company::IsValidHumanID(i)) {
				...
				if (AIConfig::GetConfig(i, AIConfig::SSS_FORCE_GAME)->GetInfo() == nullptr) {
	...
	CompanyID GetFreeSlot(int dir = 0)
	{
		...
		CompanyID slot = this->selected_slot;
		...
		switch (widget) {
			...
			case WID_AIC_INCREASE_NUMBER: {
				...
				if (widget == WID_AIC_DECREASE_NUMBER) {
					...
					new_value = std::min(static_cast<int>(MAX_COMPANIES), GetGameSettings().difficulty.max_no_competitors + 1);
				}
			...
			case WID_AIC_MOVE_UP: {
				...
				if (this->IsSelectable(this->selected_slot) && !Company::IsValidAiID(this->selected_slot) && this->IsSelectable(slot_above)) {
			...
			case WID_AIC_MOVE_DOWN: {
				...
				if (this->IsSelectable(this->selected_slot) && !Company::IsValidAiID(this->selected_slot) && this->IsSelectable(slot_below)) {
			...
			case WID_AIC_START_STOP_TOGGLE: // start or stop an AI
				if (_game_mode == GM_NORMAL && !Company::IsValidHumanID(this->selected_slot)) {
					if (Company::IsValidAiID(this->selected_slot)) { // clicking 'Stop AI'
						...
						if (this->GetCurrentNoAIs() < GetGameSettings().difficulty.max_no_competitors) {
							if (AI::CanStartNew()) {
	...
	{
		if (!this->IsSelectable(this->selected_slot)) {
			this->selected_slot = INVALID_COMPANY;
		}
		...
		this->SetWidgetDisabledState(WID_AIC_MOVE_UP, this->selected_slot == INVALID_COMPANY || Company::IsValidAiID(this->selected_slot) || GetFreeSlot(-1) == INVALID_COMPANY);
		this->SetWidgetDisabledState(WID_AIC_MOVE_DOWN, this->selected_slot == INVALID_COMPANY || Company::IsValidAiID(this->selected_slot) || GetFreeSlot(+1) == INVALID_COMPANY);
