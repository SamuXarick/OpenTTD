	...
	NWidget(WWT_PANEL, COLOUR_MAUVE, WID_AIC_BACKGROUND),
		NWidget(NWID_VERTICAL), SetPIP(4, 4, 4),
			NWidget(NWID_HORIZONTAL), SetPIP(7, 0, 7),
				NWidget(WWT_PUSHARROWBTN, COLOUR_YELLOW, WID_AIC_SN_DECREASE), SetFill(0, 1), SetDataTip(AWV_DECREASE, STR_NULL),
				NWidget(WWT_PUSHARROWBTN, COLOUR_YELLOW, WID_AIC_SN_INCREASE), SetFill(0, 1), SetDataTip(AWV_INCREASE, STR_NULL),
				NWidget(NWID_SPACER), SetMinimalSize(6, 0),
				NWidget(WWT_TEXT, COLOUR_MAUVE, WID_AIC_START_NEXT), SetDataTip(STR_AI_CONFIG_START_NEXT, STR_AI_CONFIG_START_NEXT_TOOLTIP), SetFill(1, 0), SetPadding(1, 0, 0, 0),
			EndContainer(),
			NWidget(NWID_HORIZONTAL), SetPIP(7, 0, 7),
...
struct AIConfigWindow : public Window {
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		...
		if (widget < WID_AIC_DECREASE || widget > WID_AIC_START_NEXT) {
