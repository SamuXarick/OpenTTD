...
static bool TestTownOwnsBridge(TileIndex tile, const Town *t)
{
	...
	TileIndex adjacent = tile + ReverseDiagDir(GetTunnelBridgeDirection(tile));
	...
	if (!town_owned) {
		...
		adjacent = tile + ReverseDiagDir(GetTunnelBridgeDirection(GetOtherTunnelBridgeEnd(tile)));
...
static void RemoveNearbyStations(Town *t, TileIndex tile, BuildingFlags flags)
{
	for (StationList::iterator it = t->stations_near.begin(); it != t->stations_near.end(); /* incremented inside loop */) {
		...
		if (flags & BUILDING_2_TILES_Y)   covers_area |= st->TileIsInCatchment(tile + TileIndexDiffC(0, 1));
		if (flags & BUILDING_2_TILES_X)   covers_area |= st->TileIsInCatchment(tile + TileIndexDiffC(1, 0));
		if (flags & BUILDING_HAS_4_TILES) covers_area |= st->TileIsInCatchment(tile + TileIndexDiffC(1, 1));
 ...
 *   Assuming a road from (tile - dir) to tile,
...
static bool GrowTownWithBridge(const Town *t, const TileIndex tile, const DiagDirection bridge_dir)
{
	...
	if (!(GetTownRoadBits(tile + ReverseDiagDir(bridge_dir)) & DiagDirToRoadBits(bridge_dir))) return false;
...
static bool GrowTownWithTunnel(const Town *t, const TileIndex tile, const DiagDirection tunnel_dir)
{
	...
	if (!(GetTownRoadBits(tile + ReverseDiagDir(tunnel_dir)) & DiagDirToRoadBits(tunnel_dir))) return false;
	...
	if (cur_rb == ROAD_NONE) {
		...
		switch (t1->layout) {
			...
			case TL_ORIGINAL:
				...
				if (!IsRoadAllowedHere(t1, tile + target_dir, target_dir)) {
					/* A road is not allowed to continue the randomized road,
					 *  return if the road we're trying to build is curved. */
					if (target_dir != ReverseDiagDir(source_dir)) return;

					/* Return if neither side of the new road is a house */
					if (!IsTileType(tile + ChangeDiagDir(target_dir, DIAGDIRDIFF_90RIGHT), MP_HOUSE) &&
							!IsTileType(tile + ChangeDiagDir(target_dir, DIAGDIRDIFF_90LEFT), MP_HOUSE)) {
						return;
					}
		...
		if (target_dir != DIAGDIR_END && TownAllowedToBuildRoads()) {
			switch (t1->layout) {
				...
				case TL_3X3_GRID: // Use 2x2 grid afterwards!
					GrowTownWithExtraHouse(t1, house_tile + target_dir);
					[[fallthrough]];
...
{
	TileIndex target_tile = tile + dir;
...
static void MakeTownHouse(TileIndex tile, Town *t, uint8_t counter, uint8_t stage, HouseID type, uint8_t random_bits)
{
	...
	if (size & BUILDING_2_TILES_Y)   ClearMakeHouseTile(tile + TileIndexDiffC(0, 1), t, counter, stage, ++type, random_bits);
	if (size & BUILDING_2_TILES_X)   ClearMakeHouseTile(tile + TileIndexDiffC(1, 0), t, counter, stage, ++type, random_bits);
	if (size & BUILDING_HAS_4_TILES) ClearMakeHouseTile(tile + TileIndexDiffC(1, 1), t, counter, stage, ++type, random_bits);
...
static bool CheckTownBuild2x2House(TileIndex *tile, Town *t, int maxz, bool noslope)
{
	...
	for (DiagDirection d = DIAGDIR_SE;; d++) { // 'd' goes through DIAGDIR_SE, DIAGDIR_SW, DIAGDIR_NW, DIAGDIR_END
		...
		tile2 += ReverseDiagDir(d); // go clockwise
	}
...
CommandCost CmdPlaceHouse(DoCommandFlag flags, TileIndex tile, HouseID house)
{
	...
	if (hs->building_flags & TILE_SIZE_2x1) ta.Add(tile + DIAGDIR_SW);
	if (hs->building_flags & TILE_SIZE_1x2) ta.Add(tile + DIAGDIR_SE);
...
void ClearTownHouse(Town *t, TileIndex tile)
{
	...
	if (hs->building_flags & BUILDING_2_TILES_Y)   DoClearTownHouseHelper(tile + TileIndexDiffC(0, 1), t, ++house);
	if (hs->building_flags & BUILDING_2_TILES_X)   DoClearTownHouseHelper(tile + TileIndexDiffC(1, 0), t, ++house);
	if (hs->building_flags & BUILDING_HAS_4_TILES) DoClearTownHouseHelper(tile + TileIndexDiffC(1, 1), t, ++house);
