...
static bool GrowTownWithTunnel(const Town *t, const TileIndex tile, const DiagDirection tunnel_dir)
{
	...
	if (CanRoadContinueIntoNextTile(t, tile, tunnel_dir)) {
		...
		for (uint8_t tiles = 0; tiles < 4; tiles++) {
			...
			slope_tile = AddTileIndexDiffCWrap(slope_tile, delta);
		}
	...
	do {
		...
		tunnel_tile = AddTileIndexDiffCWrap(tunnel_tile, delta);
...
static inline bool RoadTypesAllowHouseHere(TileIndex t)
{
	...
	for (const auto &ptr : tiles) {
		TileIndex cur_tile = AddTileIndexDiffC(t, ptr);
...
{
	TileIndex target_tile = AddTileIndexDiffCWrap(tile, TileIndexDiffCByDiagDir(dir));
