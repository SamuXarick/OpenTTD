...
struct TicToc {
	...
	struct State {
		...
		const std::optional<bool> max_ticks;
		uint32_t count = 0;
		uint32_t ticks = 0;
		...
		State(std::string_view name, std::optional<uint32_t> max_count = {}, std::optional<bool> max_ticks = false) : name(name), max_count(max_count), max_ticks(max_ticks)
		{
	...
	inline ~TicToc()
	{
		...
		this->state.count++;
		if (!this->state.max_ticks.value() && this->state.max_count.has_value() && this->state.count == this->state.max_count.value()) {
	...
	static void Tick(const std::string_view prefix)
	{
		for (auto state : State::GetStates()) {
			if (state->max_count.has_value() && (!state->max_ticks.value() || ++state->ticks <= state->max_count.value()) || state->count == 0) continue;
			state->OutputAndReset(state->max_ticks.value() ? "" : prefix);
		}
	}
};
