...
CommandCost StationRect::BeforeAddRect(TileIndex tile, int w, int h, StationRectMode mode)
{
	if (mode == ADD_FORCE || (w <= _settings_game.station.station_spread && h <= _settings_game.station.station_spread)) {
		...
		if (ret.Succeeded()) ret = this->BeforeAddTile(tile + TileIndexDiffC(w - 1, h - 1), mode);
		return ret;
	}
	return CommandCost();
}
...
bool StationRect::AfterRemoveRect(BaseStation *st, TileArea ta)
{
	...
	if (ta.w != 1 || ta.h != 1) empty = empty || this->AfterRemoveTile(st, ta.tile + TileIndexDiffC(ta.w - 1, ta.h - 1));
	return empty;
}
