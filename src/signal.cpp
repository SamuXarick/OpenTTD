		...
		switch (GetTileType(tile)) {
			case MP_RAILWAY: {
				...
				if (IsRailDepot(tile)) {
					if (enterdir == INVALID_DIAGDIR) { // from 'inside' - train just entered or left the depot
						...
						tile = TileAddByDiagDir(tile, exitdir);
				...
				for (DiagDirection dir = DIAGDIR_BEGIN; dir < DIAGDIR_END; dir++) { // test all possible exit directions
					if (dir != enterdir && (tracks & _enterdir_to_trackbits[dir])) { // any track incidating?
						TileIndex newtile = TileAddByDiagDir(tile, dir);  // new tile to check
			...
			case MP_TUNNELBRIDGE: {
				...
				if (enterdir == INVALID_DIAGDIR) { // incoming from the wormhole
					...
					tile = TileAddByDiagDir(tile, exitdir); // just skip to next tile
		...
		switch (GetTileType(tile)) {
			...
			case MP_ROAD:
				if ((TrackStatusToTrackBits(GetTileTrackStatus(tile, TRANSPORT_RAIL, 0)) & _enterdir_to_trackbits[dir]) != TRACK_BIT_NONE) {
					...
					_tbdset.Add(TileAddByDiagDir(tile, dir), ReverseDiagDir(dir));
					break;
				}
				[[fallthrough]];
