...
static void MakeIndustryTileBigger(TileIndex tile)
{
	...
	case GFX_OILRIG_1: {
		 ...
		 * station. */
		TileIndex other = tile + TileIndexDiffC(0, 1);
...
{
	TileIndexDiffC diff = TileOffsByAxis(OtherAxis(DiagDirToAxis(side)));
	TileIndexDiffC neighbour_diff = TileOffsByDiagDir(side);
...
static void PlantFarmField(TileIndex tile, IndustryID industry)
{
	...
	TileArea ta(tile - TileIndexDiffC(std::min(TileX(tile), size_x / 2), std::min(TileY(tile), size_y / 2)), size_x, size_y);
	ta.ClampToMap();
	...
	SetupFarmFieldFence(ta.tile + TileIndexDiffC(ta.w - 1, 0), ta.h, type, DIAGDIR_SW);
	SetupFarmFieldFence(ta.tile + TileIndexDiffC(0, ta.h - 1), ta.w, type, DIAGDIR_SE);
}
...
static bool CheckCanTerraformSurroundingTiles(TileIndex tile, uint height, int internal)
{
	...
	TileArea ta(tile - TileIndexDiffC(1, 1), 2, 2);
	for (TileIndex tile_walk : ta) {
		...
		if (internal == 0 && curh != height) {
			if (TileX(tile_walk) == 0 || TileY(tile_walk) == 0 || !CheckCanTerraformSurroundingTiles(tile_walk + TileIndexDiffC(-1, -1), height, internal + 1)) {
				return false;
			}
		}
	}
...
static bool CheckIfCanLevelIndustryPlatform(TileIndex tile, DoCommandFlag flags, const IndustryTileLayout &layout)
{
	...
	TileArea ta(tile + TileIndexDiffC(-_settings_game.construction.industry_platform, -_settings_game.construction.industry_platform),
...
static void ChangeIndustryProduction(Industry *i, bool monthly)
{
	...
	if (!suppress_message && str != STR_NULL) {
		...
		if (closeit) {
			AddTileNewsItem(str, nt, i->location.tile + TileIndexDiffC(1, 1));
		} else {
