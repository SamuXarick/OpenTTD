...
static bool DisasterTick_Submarine(DisasterVehicle *v)
{
	...
	TileIndex tile = AddTileIndexDiffC(v->tile, TileIndexDiffCByDiagDir(DirToDiagDir(v->direction)));
	if (IsValidTile(tile)) {
	...
	for (m = 0; m < 15; m++) {
		for (const Industry *i : Industry::Iterate()) {
			if ((GetIndustrySpec(i->type)->behaviour & INDUSTRYBEH_CAN_SUBSIDENCE) && --index < 0) {
				SetDParam(0, i->town->index);
				AddTileNewsItem(STR_NEWS_DISASTER_COAL_MINE_SUBSIDENCE, NT_ACCIDENT, TileAddXY(i->location.tile, 1, 1)); // keep the news, even when the mine closes
