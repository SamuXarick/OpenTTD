...
void UpdateLevelCrossing(TileIndex tile, bool sound, bool force_bar)
{
	...
	for (DiagDirection dir : { dir1, dir2 }) {
		for (TileIndex t = tile; !forced_state && t < Map::Size() && IsLevelCrossingTile(t) && GetCrossingRoadAxis(t) == axis; t += dir) {
	...
	for (DiagDirection dir : { dir1, dir2 }) {
		for (TileIndex t = tile + dir; t < Map::Size() && IsLevelCrossingTile(t) && GetCrossingRoadAxis(t) == axis; t += dir) {
...
static PBSTileInfo ExtendTrainReservation(const Train *v, TrackBits *new_tracks, DiagDirection *enterdir)
{
	...
	while (ft.Follow(tile, cur_td)) {
		...
		if (target_seen || KillFirstBit(ft.new_td_bits) != TRACKDIR_BIT_NONE) {
			...
			if (ft.tiles_skipped != 0) ft.new_tile -= TileIndexDiffCByDiagDir(ft.exitdir) * ft.tiles_skipped;
...
static Track ChooseTrainTrack(Train *v, TileIndex tile, DiagDirection enterdir, TrackBits tracks, bool force_res, bool *got_reservation, bool mark_stuck)
{
	...
	while (!IsSafeWaitingPosition(v, res_dest.tile, res_dest.trackdir, true, _settings_game.pf.forbid_90_deg)) {
		...
		TileIndex     next_tile = res_dest.tile + exitdir;
...
bool TryPathReserve(Train *v, bool mark_as_stuck, bool first_tile_okay)
{
	...
	if (v->track == TRACK_BIT_DEPOT) {
		if (HasDepotReservation(v->tile)) {
			...
			TileIndex next_tile = v->tile + GetRailDepotDirection(v->tile);
	...
	TileIndex     new_tile = origin.tile + exitdir;
					...
					if ((red_signals & chosen_track) && v->force_proceed == TFP_NONE) {
						...
						if (!HasSignalOnTrackdir(gp.new_tile, ReverseTrackdir(i))) {
							...
							if (!_settings_game.pf.reverse_at_signals || ++v->wait_counter < _settings_game.pf.wait_twoway_signal * Ticks::DAY_TICKS * 2) {
								...
								TileIndex o_tile = gp.new_tile + exitdir;
