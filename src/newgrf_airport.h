...
public:
	 ...
	 */
	AirportTileTableIterator(const AirportTileTable *att, TileIndex base_tile) : TileIterator(AddTileIndexDiffC(base_tile, att->ti)), att(att), base_tile(base_tile)
	{
	}
	...
	inline TileIterator& operator ++() override
	{
		...
		if (this->att->ti.x == -0x80) {
			...
			this->tile = AddTileIndexDiffC(this->base_tile, this->att->ti);
		}
		return *this;
	}
