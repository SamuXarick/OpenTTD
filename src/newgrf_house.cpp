...
/* virtual */ uint32_t HouseScopeResolver::GetVariable(uint8_t variable, [[maybe_unused]] uint32_t parameter, bool &available) const
{
	...
	switch (variable) {
		...
		case 0x64: {
			...
			TileIndex testtile = Map::WrapToMap(this->tile + TileIndexDiffC(x_offs, y_offs));
...
bool NewHouseTileLoop(TileIndex tile)
{
	...
	if (HasBit(hs->callback_mask, CBM_HOUSE_ANIMATION_START_STOP)) {
		...
		if (hs->extra_flags & SYNCHRONISED_CALLBACK_1B) {
			...
			if (hs->building_flags & BUILDING_2_TILES_Y)   AnimationControl(tile + TileIndexDiffC(0, 1), random);
			if (hs->building_flags & BUILDING_2_TILES_X)   AnimationControl(tile + TileIndexDiffC(1, 0), random);
			if (hs->building_flags & BUILDING_HAS_4_TILES) AnimationControl(tile + TileIndexDiffC(1, 1), random);
		} else {
...
static void DoTriggerHouse(TileIndex tile, HouseTrigger trigger, uint8_t base_random, bool first)
{
	...
	switch (trigger) {
		...
		case HOUSE_TRIGGER_TILE_LOOP_TOP:
			...
			if (hs->building_flags & BUILDING_2_TILES_Y)   DoTriggerHouse(tile + TileIndexDiffC(0, 1), trigger, random_bits, false);
			if (hs->building_flags & BUILDING_2_TILES_X)   DoTriggerHouse(tile + TileIndexDiffC(1, 0), trigger, random_bits, false);
			if (hs->building_flags & BUILDING_HAS_4_TILES) DoTriggerHouse(tile + TileIndexDiffC(1, 1), trigger, random_bits, false);
			break;
	}
}
...
void WatchedCargoCallback(TileIndex tile, CargoTypes trigger_cargoes)
{
	...
	if (hs->building_flags & BUILDING_2_TILES_Y)   DoWatchedCargoCallback(north + TileIndexDiffC(0, 1), tile, trigger_cargoes, r);
	if (hs->building_flags & BUILDING_2_TILES_X)   DoWatchedCargoCallback(north + TileIndexDiffC(1, 0), tile, trigger_cargoes, r);
	if (hs->building_flags & BUILDING_HAS_4_TILES) DoWatchedCargoCallback(north + TileIndexDiffC(1, 1), tile, trigger_cargoes, r);
}
