...
PerformanceMeasurer::~PerformanceMeasurer()
{
	...
	const uint opcodes = this->elem == PFE_GAMESCRIPT ? Game::GetInstance()->GetMaxOpsTillSuspend() : Company::Get(static_cast<CompanyID>(this->elem - PFE_AI0))->ai_instance.get()->GetMaxOpsTillSuspend();
	uint value = opcodes;
	...
	if (value != opcodes) {
		if (this->elem == PFE_GAMESCRIPT) {
			Game::GetInstance()->SetMaxOpsTillSuspend(value);
		} else {
			Company::Get(static_cast<CompanyID>(this->elem - PFE_AI0))->ai_instance.get()->SetMaxOpsTillSuspend(value);
		}
	}
}
...
static std::string_view GetGSName()
{
	...
	void DrawElementMaxOpsColumn(const Rect &r) const
	{
		...
		for (PerformanceElement e : DISPLAY_ORDER_PFE) {
			...
			if (skip > 0) {
				...
				uint value = e == PFE_GAMESCRIPT ? Game::GetInstance()->GetMaxOpsTillSuspend() : Company::Get(static_cast<CompanyID>(e - PFE_AI0))->ai_instance.get()->GetMaxOpsTillSuspend();
				DrawString(r.left, r.right, y, GetString(STR_FRAMERATE_OPS, value), TC_FROMSTRING, SA_RIGHT | SA_FORCE);
				y += GetCharacterHeight(FS_NORMAL);
				drawable--;
				if (drawable == 0) break;
			} else {
	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			case WID_FGW_CAPTION:
				...
				} else if (this->element == PFE_GAMESCRIPT) {
					return GetString(STR_FRAMETIME_CAPTION_GS_MAXOPCODE, GetGSName(), Game::GetInstance()->GetMaxOpsTillSuspend());
				}
				return GetString(STR_FRAMETIME_CAPTION_AI_MAXOPCODE, this->element - PFE_AI0 + 1, GetAIName(this->element - PFE_AI0), Company::Get(static_cast<CompanyID>(this->element - PFE_AI0))->ai_instance.get()->GetMaxOpsTillSuspend());
	...
	for (PerformanceElement e = PFE_FIRST; e < PFE_MAX; e++) {
		...
		if (e >= PFE_GAMESCRIPT && e <= PFE_AI14) {
			IConsolePrint(TC_LIGHT_BLUE, "{} times: {:.2f}ms  {:.2f}ms  {:.2f}ms  opcodes: {}",
				...
				e == PFE_GAMESCRIPT ? Game::GetInstance()->GetMaxOpsTillSuspend() : Company::Get(static_cast<CompanyID>(e - PFE_AI0))->ai_instance.get()->GetMaxOpsTillSuspend());
			printed_anything = true;
		} else {
